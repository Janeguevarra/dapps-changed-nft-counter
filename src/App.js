...
import five from "./assets/two.png";
import ten from "./assets/three.png";
import bg from "./assets/back.webp";
import clover from "./assets/clover.png";
import metamask from "./assets/metamask.svg";
import coinbase from "./assets/coinbase.svg";
import FeedbackModal from "./components/FeedbackModal";
import MintCounterField from "./components/MintCounterField";
...
function App() {
	...
	const [nftcounter, setNftCounter] = useState(1);
	...
	const [feedback, setFeedback] = useState(
		`Maximum 10 Mints, to mint your NFT.`
	);
	...
	const claimNFTs = (e, amount) => {
		...
		if (
			...
			arrAllowedAddresses.includes(blockchain.account.toLowerCase())
		) {
			...
			setFeedback(`MAXIMUM 10 Mints for your ${CONFIG.NFT_NAME}...`);
			setClaimingNft(true);
			...
			setOpen(true);
			setFeedback(
				"Sorry, the address you are trying to use is not whitelisted."
			);
		}
	};
	...
	return (
		<s.Screen image={bg}>
			...
			<s.CommentText>
				On February 3rd at 7PM PST, you will be able to mint your own Bear
			</s.CommentText>
			<s.MintBlock>
				<s.Card>
					<s.CardText>
						<s.MintText>MINT</s.MintText>
						<s.MintNumber>
							<p style={{fontSize: "20px"}}>0/9999</p>
							<s.GrayText>REMAINING</s.GrayText>
						</s.MintNumber>
					</s.CardText>
					<s.GrayText>SOLD OUT</s.GrayText>
					<s.EthDetail>
						<s.MintNum>1</s.MintNum>
						<s.MintMax>2 MAX</s.MintMax>
						<s.EthValue>0.22 ETH</s.EthValue>
					</s.EthDetail>
					<s.SoldButton>SOLD OUT</s.SoldButton>
					{/* <div
						style={{
							padding: "1rem 0",
							margin: "auto",
